...
def main(args=None):
    ...
    try:
        ...
        print("Takeoff atitude reached. Holding pos for 5 seconds")
        node.get_logger().info("takeoff atitude reached. Holding pos for 5 seconds")
        time.sleep(5.0)
        if not node.switch_mode_with_timeout("RTL", rclpy.duration.Duration(seconds=20)):
            raise RuntimeError("Unable to switch to RTL")
